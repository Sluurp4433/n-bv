...
// (inte en bild) och renderar samma figur som SVG i valfri storlek.

export type AvatarConfig = {
  v: 1
  kind: string // figurtyp: 'woman' | 'man' | djur | insekt
  skin: string
  face: string
  eyes: string
  brows: string
  ...
  bodyColor: string // djur/insekter
}

// ---- Figurtyper ----
export const HUMAN_KINDS = ['woman', 'man'] as const
export const ANIMAL_KINDS = ['cat', 'dog', 'fox', 'bear', 'rabbit', 'owl', 'panda'] as const
export const INSECT_KINDS = ['bee', 'ladybug', 'butterfly', 'ant'] as const
export const ALL_KINDS = [...HUMAN_KINDS, ...ANIMAL_KINDS, ...INSECT_KINDS] as const
...
export function isAnimal(kind: string) { return (ANIMAL_KINDS as readonly string[]).includes(kind) }
export function isInsect(kind: string) { return (INSECT_KINDS as readonly string[]).includes(kind) }
...
// Kropps-/pälsfärger för djur & insekter
...
export function randomAvatar(): AvatarConfig {
  ...
  const humanHair = kind === 'man'
    ? pick(['short', 'buzz', 'side', 'spiky', 'curly'])
    : pick(['long', 'bob', 'ponytail', 'wavy', 'braids', 'pixie', 'updo', 'curly'])
  return {
    v: 1,
    kind,
    skin: pick(skinKeys),
...
export function sanitizeAvatar(input: unknown): AvatarConfig {
  ...
  return {
    v: 1,
    kind: valid(o.kind, ALL_KINDS, DEFAULT_AVATAR.kind),
